...
hc_inercia <- function(data, nombre.archivo, titulos = c(
  "Inercia", "Inercia Inter-Clase", "Inercia Intra-Clase")) {
...
#' @importFrom highcharter hchart hc_add_series hc_plotOptions hc_exporting hc_chart hc_xAxis hc_yAxis
#' 
hc_mapa <- function(pca.model, clusters, ejes = c(1, 2), colores = NULL) {
...
#' 
plotly_mapa <- function(pca.model, clusters, ejes = c(1, 2, 3), colores = NULL) {
  dims <- paste0("Dim.", ejes)
  inercias <- round(pca.model$eig[ejes, 2], digits = 2)
...
#' @importFrom highcharter hchart hc_add_series hc_plotOptions hc_exporting hc_chart hc_xAxis hc_yAxis
#' 
hc_horiz <- function(centros, nombre.archivo = NULL, colores = NULL) {
...
#' @importFrom highcharter hchart hc_add_series hc_plotOptions hc_exporting hc_chart hc_xAxis hc_yAxis
#' 
hc_vert <- function(centros, nombre.archivo = NULL, colores = NULL) {
  data <- data.frame(t(centros))
  data <- round(data, 2)
  colnames(data) <- paste0("Cluster", 1:ncol(data))
  data$vars <- row.names(data)
  ...
  for (i in 1:(ncol(data) - 1)) {
    r <- r %>% 
      e_bar_(colnames(data)[i], name = colnames(data)[i])
  }
...
#' @importFrom highcharter hchart hc_add_series hc_plotOptions hc_exporting hc_chart hc_xAxis hc_yAxis
#' 
hc_cat <- function(clusters, var, colores = NULL, escalar = T) {
